  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.messagesFetched) {
      this.setState({
        ...
        readDataSource: this.state.readDataSource.cloneWithRows(
          nextProps.messages.has_read_messages
        )
      })
    }
  }
      ...
      <View style={styles.topicCell}>
        <View style={styles.cellRow}>
          ...
          <View style={styles.cellSubtitle}>
            ...
            <Text style={styles.cellMintitleText}>
              {message.reply.create_at}
            </Text>
          </View>
          <TouchableOpacity
            style={styles.cellAccessory}
            onPress={() =>
              navigate('reply', {
                id: message.topic.id,
                replyTo: { ...message.reply, author: message.author }
              })
            }
          >
        ...
        <View style={styles.cellTitle}>
          <Text style={styles.cellSubtitleText}>
            评论了<Text style={styles.repliedTopicTitle}>
              {message.topic.title}
            </Text>
          </Text>
        </View>
        <View style={[styles.cellTitle]}>
          <HtmlView
            value={message.reply.content.replace(/(\n|\r)+$/g, '')}
            style={htmlStyles}
          />
        </View>
      </View>
    ...
    return (
      <ScrollableTabView renderTabBar={renderTabBar}>
        <ListView
          ...
          dataSource={this.state.unreadDataSource}
          renderRow={this.renderRow}
        />
        <ListView
          enableEmptySections={true}
          dataSource={this.state.readDataSource}
          renderRow={this.renderRow}
        />
      </ScrollableTabView>
    )
