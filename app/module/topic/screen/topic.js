...
class Topic extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.topicFetched || nextProps.agreeToggled) {
      this.setState({
        replyDataSource: this.state.replyDataSource.cloneWithRows(
          nextProps.topic.replies
        )
      })
    }
    ...
    return (
      <View style={[styles.topicComment]}>
        <View style={styles.topicCommentBreif}>
          ...
          <TouchableOpacity
            style={styles.topicCommentBadge}
            onPress={() => {
          ...
          <TouchableOpacity
            style={[styles.topicCommentBadge, styles.topicAgreeBadge]}
            onPress={() => this.toggleAgree(reply.id)}
          >
            <Icon
              name="thumbs-up"
              size={15}
              color={reply.agreeStatus === 'up' ? selectedIcon : unselectedIcon}
            />
            <Text
              style={[
                styles.topicAgreeBadgeText,
                {
                  color:
                    reply.agreeStatus === 'up' ? selectedIcon : unselectedIcon
                }
              ]}
            >
              {' '}
    ...
    return (
      <View style={styles.topicContent}>
        <View style={styles.topicBreif}>
          ...
          <View style={styles.topicSubtitle}>
            ...
            <Text style={styles.topicMintitleText}>
              {topic.create_at},{topic.visit_count} 次点击
            </Text>
          </View>
        ...
        <View style={[styles.topicComments]}>
          <Text style={[styles.topicCommentsStatus]}>
            {topic.reply_count} 回复 | 最后回复: {topic.last_reply_at}
          </Text>
        </View>
      </View>
    )
  ...
  renderHeaderButtons() {
    ...
    return (
      <View style={[styles.navigationBarButton, styles.headerButtons]}>
        <TouchableOpacity
          onPress={this.toggleCollect}
          style={styles.headerButton}
        >
          <Icon
            name={topic.is_collect ? 'heart' : 'heart-o'}
            size={20}
            color={topic.is_collect ? selectedIcon : unselectedIcon}
          />
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => this.toReply(topic.id)}
          style={styles.headerButton}
        >
    ...
    return (
      <View style={styles.container}>
        <Header
          ...
          rightButton={this.renderHeaderButtons()}
        />
        <ListView
